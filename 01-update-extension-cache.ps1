...
function Get-HighestLastUpdated {
    ...
    $lastUpdated = @($Extensions) |
        ...
        Sort-Object -Property @{ Expression = { [System.DateTimeOffset]$_.lastUpdated } } -Descending |
...
if ((-not (Test-Path -path $cacheFile -PathType Leaf)) -or (-not $skipCache)) {
    ...
    $highWatermark = Get-HighestLastUpdated -Extensions $cachedExtensions
    if ($highWatermark) {
        write-output "Fetching extensions updated after $($highWatermark.ToString("O"))"
    }

    do {
        $ProgressPreference = "silentlycontinue"
        $result = Import-Extensions -PageSize $pageSize -Page $page
        $resultExtensions = @($result.extensions)
        $totalFetched += $resultExtensions.Count
        ...
        $newOrUpdatedExtensions = $resultExtensions | Where-Object {
            (-not $highWatermark) -or ([System.DateTimeOffset]$_.lastUpdated -gt $highWatermark)
        }
    ...
    foreach ($extension in $cachedExtensions) {
        $extensionsById[(Get-ExtensionCacheKey -Extension $extension)] = $extension
    }
    foreach ($extension in $extensionsToProcess) {
        $extensionsById[(Get-ExtensionCacheKey -Extension $extension)] = $extension
    }

    $extensions = @($extensionsById.Values)

    # Sort the extensions to prevent unwanted cache commits
    $extensions = $extensions | Sort-Object -Property extensionId
    Set-Content -path $cacheFile -Value (ConvertTo-Json -InputObject @($extensions) -Depth 100)
    write-commit -message "Update extensions cache"

    write-output "Processing $($extensionsToProcess.Count) updated extensions"
}
else {
